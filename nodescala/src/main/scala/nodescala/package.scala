...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def always[T](value: T): Future[T] = {
      /**
       * a promise can be used to successfully complete a future
       * with a value (by “completing” the promise) using the success method
       */
      val p = promise[T]
      p complete (Try(value)) // equal to p success value
      p.future
    }
    ...
    def never[T]: Future[T] = {
      // a promise which is never fulfilled
      // its future is never completed
      val p = promise[T]
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = Try(Await.result(f, 0 second)) match {
      case Success(x) => x
      case Failure(t) => throw new NoSuchElementException
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]
      f.onComplete {
        case t: Try[T] => p complete Try(cont(f))
      }
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]
      f.onComplete {
        case t: Try[T] => p complete Try(cont(t))
      }
      p.future
    }
  ...
  object CancellationTokenSource {
    ...
    def apply(): CancellationTokenSource = new CancellationTokenSource {
      val p = Promise[Unit]()
      val retCancellationToken = new CancellationToken {
      ...
      def cancellationToken = retCancellationToken
    }
  }
